...
(function() {
    ...
    function addDownloadPanel(container, videoId) {
        ...
        btnDownload.onclick = () => {
            ...
            fetch(`${LOCAL_SERVER}/download?v=${videoId}&quality=${quality}`)
                .then(res => res.ok ? res.json() : Promise.reject('Сервер не відповідає'))
    ...
    function initialize() {
        ...
        // Використовуємо інтервал для надійного пошуку контейнера
        let attempts = 0;
        const intervalId = setInterval(() => {
            ...
            // Зупиняємо спроби через 15 секунд, щоб не навантажувати систему
            if (attempts > 30) {
                clearInterval(intervalId);
                console.log('YT Download Script: не вдалося знайти контейнер для кнопок.');
            }
